...
function MerchantRow({ merchant, copies, itemId, isMerchant, item, addMargin, setMargin, margin }) {
    const [sended, setSended] = useState(false);
    console.log(isMerchant)
    const sizeCopy = copies.filter(c => c.merchant == merchant.id);

    let total = sizeCopy[0]?.quoted_price + sizeCopy[0]?.quoted_price * margin / 100;

    async function handleSend() {
        const supabase = createClient();

        // const item = await supabase.from("item").update({ merchant: merchant.id, status: "send" }).eq("id", itemId);
        const itemcopy = await supabase.from("itemcopy").insert({ item: itemId, merchant: merchant.id, comment: "", part_no: item.part_no, position_no: item.position_no, alternative_part_no: item.alternative_part_no, quoted_price: 0, quoted: false })
        ...
        <TableRow>
            <TableCell colSpan={8}>
                <div>
                    ...
                    <div className="mt-4">
                        <Card className={(sizeCopy.length || isMerchant) && `bg-emerald-100 dark:bg-emerald-950/80 dark:border dark:border-emerald-500`}>
                            <CardHeader>
                                        ...
                                        <>
                                            <div className="flex gap-2">
                                                        ...
                                                        <DialogContent>
                                                            ...
                                                            <div className="grid gap-2">
                                                                <div>
                                                                    ...
                                                                    <Input type="text" value={sizeCopy[0].part_no} disabled />
                                                                </div>
                                                                <div>
                                                                    <Label>
                                                                        Position No
                                                                    </Label>
                                                                    <Input type="text" value={sizeCopy[0].position_no} disabled />
                                                                </div>
                                                                <div>
                                                                    <Label>
                                                                        Alternative Part No
                                                                    </Label>
                                                                    <Input type="text" value={sizeCopy[0].alternative_part_no} disabled />
                                                                </div>
                                                                <div>
                                                                    <Label>
                                                                        Comment
                                                                    </Label>
                                                                    <Input type="text" value={sizeCopy[0].comment.split("[OFFER]")[0]} disabled />
                                                                </div>
                                                            </div>
                                                ...
                                                <div>
                                                    ...
                                                    <Input value={sizeCopy[0].quoted_price} disabled className="bg-white text-black" />
                                                </div>
                                                <div>
                                                    <Label className="text-xs">Add Margin(%)</Label>
                                                    <Input value={margin} type="number" className="bg-white text-black" onChange={e => setMargin(e.target.value)} disabled={item.status == "finalized"} />
                                                </div>
                                                <div>
                                                    <Label className="text-xs">Total Price</Label>
                                                    <Input value={total} type="number" className="bg-white text-black" disabled />
                                                </div>
                                                {item.status !== "finalized" && (
                                                    <div className="self-end">
                                                        <Button onClick={() => addMargin(margin, sizeCopy[0].quoted_price, merchant.id, sizeCopy[0].comment.split("[OFFER]")[1])}>
                                                            Finalize
                                                        </Button>
                                                    </div>
                                                )}
...
function ItemRow({ item, merchants, i }) {
    ...
    async function addMargin(margin, quoted_price, marchantId, offered_qty) {
        ...
        await supabase.from("item").update({ margin: margin, quoted_price: quoted_price, quoted: true, merchant: marchantId, offered_qty: Number(offered_qty) }).eq("id", item.id);
    ...
    async function fetchCopies() {
        ...
        const itemsCopies = await supabase.from("itemcopy").select("*").eq("item", item.id);
    ...
    return (
        <>
            <TableRow>
                ...
                <TableCell className="text-right">{item.quoted_price}</TableCell>
                {/* {item.quoted && !(item.status == "finalized") ? (
                    <TableCell className="text-right">
                        <Label htmlFor="margin">
                            Enter Margin
                        </Label>
                        <Input type="number" name="margin" value={margin} onChange={(e) => setMargin(Number(e.target.value))} />
                        <Button onClick={addMargin}>
                            Add
                        </Button>
                    </TableCell>
                ) : (
                    <TableCell className="text-right">
                        {item.status == "finalized" ? margin : "Waiting for Vendor to Quote"}
                    </TableCell>
                )} */}
            </TableRow>
            <TableRow>
                <TableCell colSpan={9}>
                    <div className="flex w-full justify-between items-center">
                        ...
                        <Button onClick={() => setOpenMerchant(!openMerchant)} className="w-8 h-8 rounded-full">
                            <ChevronDown className={`${openMerchant && "rotate-180"} transition duration-300 h-6 w-6`} />
                        </Button>
                    </div>
                </TableCell>
            </TableRow>
            {openMerchant && (
                merchants?.map((merchant, i) =>
                    <MerchantRow merchant={merchant} item={item} itemId={item.id} copies={copies} isMerchant={merchant.id == item.merchant} key={i} addMargin={addMargin} margin={margin} setMargin={setMargin} />
                )
            )}
        </>
    )
...
export default function RFQDetailPage({
    ...
    const allmerchant = merchants?.filter((m) => m.brands.includes(rfq.brand) || m.category.includes(rfq.category) || m.model.includes(rfq.model))
    ...
    async function sendQuote() {
        ...
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            await supabase.from("item").update({ status: "finalized" }).eq("id", item.id);
        }
    ...
    return (
        <main className="grid">
            ...
            {(items?.length == filterItem?.length) && (
            ...
            {(items?.length == filterItem?.length) && (
                <div className="my-4 flex justify-center">
                    <Button onClick={sendQuote} disabled={finalItem?.length == items?.length}>
                        {finalItem?.length == items?.length ? "Quote Sended" : "Send Quote"}
                    </Button>
                </div>
            )}
        </main>
