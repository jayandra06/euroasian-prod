...
function ItemTable({ itemData, i }: { itemData: any; i: number }) {
  ...
  const [alternative, setAlternative] = useState({
    ...
    position_no: itemData.position_no,
    alternative_part_no: itemData.alternative_part_no,
  ...
  async function handleQuote() {
    ...
    const itemDat = await supabase
      .from("itemcopy")
      .update({
        quoted: true,
        quoted_price: price,
        part_no: alternative.part_no,
        position_no: alternative.position_no,
        alternative_part_no: alternative.alternative_part_no,
        comment: `${alternative.comment}[OFFER]${offer}`,
      })
      .eq("id", itemData.id);
            ...
            <div className="col-span-4 flex gap-2">
              <div className="grid grid-cols-4 items-center gap-4">
                <Input
                  ...
                  onChange={(e) =>
                    setAlternative({ ...alternative, comment: e.target.value })
                  }
              ...
              <Dialog>
                <DialogTrigger>
                  <Button>Add Alternate Item</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>Add Alternate Item</DialogTitle>
                    <DialogDescription>Create Alternate Item</DialogDescription>
                  </DialogHeader>
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        id="vesselName"
                        placeholder="Enter Part No..."
                        value={alternative.position_no}
                        onChange={(e) =>
                          setAlternative({
                            ...alternative,
                            position_no: e.target.value,
                          })
                        }
                        className="col-span-3"
                      />
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        id="vesselName"
                        placeholder="Enter Part No..."
                        value={alternative.alternative_part_no}
                        onChange={(e) =>
                          setAlternative({
                            ...alternative,
                            alternative_part_no: e.target.value,
                          })
                        }
                        className="col-span-3"
                      />
                  ...
                  <DialogFooter>
                    <Button type="submit" onClick={addAlternateItem}>
                      Add Item
                    </Button>
                  </DialogFooter>
                </DialogContent>
...
export default function VendorRfqs() {
  ...
  async function fetchItems() {
    ...
    const profileData = await supabase
      ...
      .eq("id", user!.id)
      .single();
    const merchantData = await supabase
      .from("merchant")
      .select("*")
      .eq("merchant_profile", profileData.data.id)
      .single();
    ...
    const rfqs = await supabase
      .from("rfq")
      .select("*")
      .contains("suppliers", merchantData.data!.id);
  }
      ...
      <main className="grid max-w-6xl w-full justify-self-center gap-4">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <h1 className="text-4xl font-bold">{items?.length}</h1>
            </CardContent>
            <CardFooter>
          ...
          <Card>
            ...
            <CardContent>
              <h1 className="text-4xl font-bold">{quotedItems?.length}</h1>
            </CardContent>
            <CardFooter>
          ...
          <Card>
            ...
            <CardContent>
              <h1 className="text-4xl font-bold">
                {items?.length &&
                  quotedItems?.length &&
                  items?.length - quotedItems?.length}
              </h1>
            </CardContent>
            <CardFooter>
