...
export default function BrandsPage() {
    ...
    async function requestAddBrand() {
        ...
        if (error) {
            ...
            alert("Failed to request the new brand. Please check the console.");
        } else {
            alert(`${newBrand.name} has been submitted for approval.`);
    ...
    async function requestUpdateBrand() {
        ...
        const { error } = await supabase
            .from("brand")
            .update({ name: editBrand.name, description: editBrand.description, is_active: false }) // Set to pending after update
            .eq("id", editBrand.id);

        if (error) {
            console.error("Error updating brand:", error);
            alert("Failed to request update for the brand details. Please check the console.");
        } else {
            alert(`${editBrand.name} update has been submitted for approval.`);
            ...
            <Dialog open={editBrandDialogOpen} onOpenChange={setEditBrandDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Update the details for the selected brand. This will be submitted for admin approval.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="grid gap-4 py-4">
                        <div className="grid gap-2">
